 ...
 * Scorecard Service
 *
 * API client for scorecard domain.
...
const rawScorecardService = {
  /**
   * List scorecards
   */
  async getScorecard(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/scorecards` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getScorecard(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/scorecards`;
   ...
   * Get scorecard by id
   */
  async getScorecard(scorecardId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/scorecards/${scorecardId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getScorecard(initiativeId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/initiatives/${initiativeId}/scorecard` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const scorecardService = makeService(rawScorecardService, "scorecard");
